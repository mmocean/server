...
{
	SERVER_DEBUG( "pthread_exit_wrapper\n" );
	assert( NULL != pv );
...
{	
	SERVER_DEBUG( "pthread_join_wrapper:\n" );
	assert( NULL != pv );
	
	const struct pthread_vargs *p = pv;
	while( NULL != p )
	{
		int nRet = 0;
		if( ( nRet = pthread_join( p->pid, NULL ) ) < 0 )
		{
			SERVER_DEBUG( "pthread_join error:%s\n", strerror(nRet) );
			return -1;	
		}
		p = p->next;
	}
...
{
	SERVER_DEBUG( "####pthread_vargs_display####\n" );
	assert( NULL != pv );
	...
	SERVER_DEBUG( "# read_from_downstream:%ld bytes\n", pv->read_from_downstream );
	SERVER_DEBUG( "# write_to_downstream: %ld bytes\n", pv->write_to_downstream );
	SERVER_DEBUG( "# read_from_upstream:  %ld bytes\n", pv->read_from_upstream );
	SERVER_DEBUG( "# write_to_upstream:   %ld bytes\n", pv->write_to_upstream );
}
...
{
	SERVER_DEBUG( "pthread_vargs_cleanup:\n" );
	assert( NULL != pv );

	while( NULL != pv )
	{
		const struct pthread_vargs *p = pv->next;
		pthread_vargs_display( pv );
		free( (void*)pv );
		pv = p;
	}
	pv = NULL;
